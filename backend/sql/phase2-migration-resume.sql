-- Phase 2 Migration - RESUME VERSION
-- This skips parts that already exist and continues from where you left off

-- ===========================================
-- STEP 1: Check what already exists (SKIP - enum already created)
-- ===========================================
-- The billing_status_enum already exists, so we skip creating it

-- ===========================================
-- STEP 2: Add billing status column as TEXT first (SKIP if already exists)
-- ===========================================
ALTER TABLE public.sessions
ADD COLUMN IF NOT EXISTS billing_status TEXT;

-- ===========================================
-- STEP 3: Add other columns (SKIP if already exist)
-- ===========================================
ALTER TABLE public.sessions
...
-- ===========================================
-- STEP 5: Convert billing_status to enum type (SKIP if already converted)
-- ===========================================
...
BEGIN
    IF EXISTS (
        ...
        RAISE NOTICE 'billing_status is already enum type, skipping conversion';
    ELSE
        -- Drop any existing default to avoid cast issues
        ALTER TABLE public.sessions ALTER COLUMN billing_status DROP DEFAULT;
        ...
        RAISE NOTICE 'billing_status converted to enum type';
    END IF;
END $$;

-- ===========================================
-- STEP 6: Add constraint for logical amounts (SKIP if already exists)
-- ===========================================
ALTER TABLE public.sessions
...
-- ===========================================
-- STEP 7: Create insurance portals table (SKIP if already exists)
-- ===========================================
...
CREATE INDEX IF NOT EXISTS idx_portals_payer ON public.insurance_portals(payer_id);
ALTER TABLE public.insurance_portals ENABLE ROW LEVEL SECURITY;

-- Drop existing policy if it exists, then recreate
DROP POLICY IF EXISTS "Admins and billing can view portals" ON public.insurance_portals;
...
-- ===========================================
-- STEP 8: Create payments tables (SKIP if already exist)
-- ===========================================
...
CREATE INDEX IF NOT EXISTS idx_payment_sessions_payment ON public.payment_sessions(payment_id);
CREATE INDEX IF NOT EXISTS idx_payment_sessions_session ON public.payment_sessions(session_id);

ALTER TABLE public.payments ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.payment_sessions ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Admins and billing can manage payments" ON public.payments;
...
DROP POLICY IF EXISTS "Admins and billing can manage payment_sessions" ON public.payment_sessions;
...
-- ===========================================
-- STEP 9: Create status history table (SKIP if already exists)
-- ===========================================
...
CREATE INDEX IF NOT EXISTS idx_status_history_session ON public.session_status_history(session_id);
CREATE INDEX IF NOT EXISTS idx_status_history_date ON public.session_status_history(changed_at);

ALTER TABLE public.session_status_history ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "All authenticated users can view status history" ON public.session_status_history;
CREATE POLICY "All authenticated users can view status history"
ON public.session_status_history FOR SELECT
USING (auth.role() = 'authenticated');

-- ===========================================
-- STEP 10: Create alerts table (SKIP if already exists)
-- ===========================================
...
DROP POLICY IF EXISTS "Users see assigned alerts" ON public.alerts;
...
-- ===========================================
-- STEP 11: Create helpful views (REPLACE if they exist)
-- ===========================================
...
-- ===========================================
-- STEP 12: Create triggers (REPLACE if they exist)
-- ===========================================
...
DROP TRIGGER IF EXISTS sessions_status_change ON public.sessions;
